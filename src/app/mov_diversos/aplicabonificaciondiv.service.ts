...
@Injectable()
export class AplicaBonificServiceDiv {

  //propiedades de la clase
  private aplicabonificacionUrl: string;
  private id_movdiversos:number;
  ...
  constructor (private http: Http,
      ...
      private alertService: AlertService)
      {
        ...
        this.aplicabonificacionUrl=String(this.url.getUrlAplicabonificacionDiv());

      }

  postBonificaciones(id_movdiversos:number, id_benef:number, imp_cap:number, imp_int:number, imp_adm:number,
  ...
  serie:string, id_catprog:number): Observable<Bonific[]> {
    ...
    for (var i = 0; i < 1 ; i++) {
      ...
      id_movdiversos = this.currentUser[i].id_mov_diversos;
    }

    console.log("numero de contrato dentro de aplicabonificacion.service: " + this.numcontrato);
    console.log("id_movdiversos dentro de aplicabonificacion.service "+id_movdiversos);


    //metodo callback para introducir los datos al backend console
    /*console.log(this.aplicabonificacionUrl+id_movdiversos.toString()+'&id_benef='+id_benef.toString()+'&imp_cap='+imp_cap.toString()+'&imp_int='+imp_int.toString()+'&imp_adm='+imp_adm.toString()+
    ...
    //metodo callback para introducir los datos al backend
     return this.http.get(this.aplicabonificacionUrl+id_movdiversos.toString()+'&id_benef='+id_benef.toString()+'&imp_cap='+imp_cap.toString()+'&imp_int='+imp_int.toString()+'&imp_adm='+imp_adm.toString()+
     '&imp_seg='+imp_seg.toString()+'&imp_osg='+imp_osg.toString()+'&imp_com='+imp_com.toString()+'&imp_mor='+imp_mor.toString()+'&imp_tit='+imp_tit.toString()+'&id_catbonific='+id_catbonific.toString()+
     '&estatus='+estatus+'&id_usuario='+id_usuario.toString()+'&clave_b='+clave_b+'&recibo='+recibo.toString()+'&serie='+serie+'&numcontrato='+this.numcontrato+'&id_catprog='+id_catprog.toString()+
     '&id_autoriza='+id_autoriza.toString())
  ...
  private extractDataBon(res: Response) {
    let body= res.json();
    console.log("JSON DE REGRESO despues de pedir la BONIFICACION" +body.registroBonificacionDiv);
    let id_mov_diversos= 0;
    let id_bonificacion = 0;
    let actividad: boolean;
    let recibo= 0;
    for (var i = 0; i < body.registroBonificacionDiv.length; ++i) {
      id_mov_diversos =  body.registroBonificacionDiv[0];
      recibo =body.registroBonificacionDiv[1];
      id_bonificacion = body.registroBonificacionDiv[2];
      actividad = body.registroBonificacionDiv[3];
    ...
    return body.registroBonificacionDiv || { };
